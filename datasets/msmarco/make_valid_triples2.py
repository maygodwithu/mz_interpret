...
def generate_triples(outfile, triples_to_generate):
    ...
    unjudged_rank_to_keep = random.sample(range(100), 5)
    ...
    with gzip.open("./data/msmarco-docdev-top100.gz", 'rt', encoding='utf8') as top100f, \
        ...
        for line in top100f:
            ...
            if(before_topicid != topicid):
                ...
                #unjudged_rank_to_keep = random.sample(range(100), 5)
                unjudged_rank_to_keep = random.sample(range(100), 9)
                before_topicid = topicid
                triples_to_generate -= 1
                if triples_to_generate <= 0:
                    return stats

            if int(rank) not in unjudged_rank_to_keep:
                stats['skipped'] += 1
                continue
...
stats = generate_triples("/home/jkchoi/.matchzoo/datasets/msmarco/msmarco-dev5.tsv", 5192)
