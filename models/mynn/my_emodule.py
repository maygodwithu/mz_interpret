...
from .my_edefine import _MaxPoolNd, _ConvNd, _ntuple, _pair, _NormBase, _AvgPoolNd
...
class my_Linear(nn.Module):
    def __init__(self, in_features, out_features, bias=True):
        ...
        self._everbose = True
        self._ovalue = None    ## original forward value
        self._value = None     ## save max value
    ...
    def forward(self, input):
        ...
        if(self._mode == 2):  ## find path
            ...
            self._ovalue = x.clone() ## save origianl value
            return x
    ...
    def getOValue(self):  ## return output
        return self._ovalue

    def backward(self, input, theta, pre_out):
    ...
    def e_backward(self, input, theta, pre_out):
        ...
        for i in range(input.shape[1]):
            ...
            tpos = 0
            if(nv >= 0):
                tv, tp = torch.sort(pre_out*tweight, descending=True)
                for tvi in tv:
                    tsum += tvi
                    tpos += 1
                    if(tsum >= nv): break
            else:
                tv, tp = torch.sort(pre_out*tweight)
                for tvi in tv:
                    tsum += tvi
                    tpos += 1
                    if(tsum < nv): break
            ...
            if False:
                ...
                print('tweight shape =', tweight.shape)
                print("out pos = ", nr)
        ...
        prev = torch.index_select(pre_out, 0, selp)
...
class my_ReLU(nn.Module):
    def __init__(self, inplace=False):
        ...
        self._everbose = True  ## for e-backward
    ...
    def e_backward(self, input, theta, pre_out):
        if self._everbose:
            print('=== Relu ebackwrd ===')
            print('!now nothing done. but highly recommend to check ReLU value is 1')
        return input
 
class my_Conv1d(_ConvNd):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
        ...
        self._ovalue = None   ## save original value
        self._index = None
        self._value = None
        self._verbose = False
        self._everbose = True
    ...
    def find_conv_pos(self, current_pos, fake_input, tweight, Fix=False):
        ...
        if Fix:
            #rpos = 1 ## cdssm case filter size = 3
            rpos = ts[2] // 2 ## cdssm case filter size = 3
            ws = self.getOutShape()
            under_pos = current_pos - out_channel * ws[2]
            return rpos, under_pos

    def make_weightxinput(self):
        ws = self.weight.shape
        p = torch.zeros(self.weight[0].shape).flatten()      ## candidate
    ...
    def get_input_position(self, pos):
        c_size = self.input_shape[2]
        tz = pos // c_size
        tx = pos % c_size
        return tz, tx

    def modify_position(self, org, mv_x, diff):
        new_val = org + diff
        z, x = self.get_input_position(org)  
        new_x = x + mv_x
    ...
    def modify_pv(self, p, rpos, under_pos):
        under_z, under_x = self.get_input_position(under_pos)
        zero_z, zero_x  = self.get_input_position(int(p[rpos].item()))
        ...
        diff = under_pos - int(p[rpos].item()) # 

        ### new approach
        p = p + diff
        ws = self.weight.shape
        for i in range(ws[2]):
             outrange = self.modify_position(p[i]-diff, mv_x, diff) # aleady plus diff
             if(outrange == -1):
                 z, x = self.get_weight_position(i)  
                 #print(outrange, p[i], mv_x, mv_y, diff, i, z, x, y, file=sys.stderr)
                 p.reshape(ws[1], ws[2])[:,x] = -1
    ...
    def e_backward(self, input, theta, pre_out): 
        ...
        for i in range(input.shape[1]):
            ...
            ip = p.clone()   # <0 invalid
            ip[ip<0] *= 0   # <0 invalid
            sel_val = torch.index_select(pre_out, 0, ip.long())
            sel_val *= zero_p 
            #3. find effective path 
            out_channel = self.getOutchannel(nr)
            tsum = torch.zeros(1)
            tpos = 0
            if(nv >= 0):
                tv, tp = torch.sort(sel_val*tweight[out_channel].flatten(), descending=True)
                for tvi in tv:
                    tsum += tvi
                    tpos += 1
                    if(tsum >= nv): break
            else:
                tv, tp = torch.sort(sel_val*tweight[out_channel].flatten())
                for tvi in tv:
                    tsum += tvi
                    tpos += 1
                    if(tsum < nv): break
            ...
            if(selp is None):
                selp = p[ tp[:tpos] ].data  ## origianl value
            else:
            ...
            if False:
                ...
                print('weight shape =', self.weight.shape)
                print('tweight[channel] shape =', tweight[out_channel].shape)
                print('pre_out shape =', pre_out.shape)
                print("out pos = ", nr)
                print('tv shape =', tv.shape)
                print('limit value =', nv)
        ...
        curv = torch.index_select(cur_out, 0, selp.long())
        nrs = torch.stack([selp.float(), prev, curv])
...
class my_Conv2d(_ConvNd):
    ...
    def make_fakeinput(self, rpos):
        ts = self.weight.shape
        fake_input = torch.zeros(self.input_shape)
        in_channel = rpos // (ts[2] * ts[3])  ## input channel
        n=in_channel * (self.input_shape[2] * self.input_shape[3])
        for px in range(self.input_shape[2]):
            for py in range(self.input_shape[3]):
                n += 1  ## for 0.. we cannot discriminate
                fake_input[0,in_channel,px,py] = n
        return fake_input
    ...
    def find_conv_pos(self, current_pos, fake_input, tweight, Fix=False):
        ...
        if Fix:
            ws = self.getOutShape()
            #rpos = 4 ## 'same' conv case
            rpos = (ts[2]*ts[3]) // 2
            under_pos = current_pos - out_channel * (ws[2]*ws[3])
            return rpos, under_pos

        ##To find rpos
        under_pos = 0
        for rpos in range(ts[2]*ts[3]):
            tweight *= 0
            tweight[out_channel].flatten()[rpos] = 1.0
            fake_conv = self._my_conv_forward(fake_input, tweight)
            under_pos = fake_conv.flatten()[current_pos].item()
            if(under_pos > 0): break   ## find rpos!
        under_pos -= 1  ## start from 1 --> 0 
        if self._everbose:
    ...
    def get_weight_position(self, pos):
        ws = self.weight.shape
        c_size = ws[2]*ws[3]
        tz = pos // c_size
        residual = pos % c_size
    ...
    def modify_pv(self, p, rpos, under_pos):
        ...
        #if self._everbose:
        if False:
            print('=conv small iter = ')
            print('rpos = ', rpos)
            print('zero pos = ', p[rpos])
            print('under pos = ', under_pos)
            print('zero_z  = ', zero_z, 'zero_x  = ', zero_x, 'zero_y  = ', zero_y)
            print('under_z = ', under_z, 'under_x = ', under_x, 'under_y = ', under_y)
            print('mv_x = ', mv_x, 'my_y = ', mv_y)
        ...
        ### new approack end
        #for wn in range(len(p)):
            #p[wn] = self.modify_position(int(p[wn].item()), mv_x, mv_y, diff)
...
class _BatchNorm(_NormBase):
    ...
    def e_backward(self, input, theta, pre_out):
        #1. change value of nrs --> value of pre_out
        pre_out = pre_out.flatten()
        nrs = input.clone()
        nrs[1,:] = pre_out[ nrs[0,:].int() ]

        if self._everbose:
            print('=== batchnorm1 ebackwrd ===')
            print('change value confirm..')
        return nrs

class my_BatchNorm1d(_BatchNorm):
...
class my_MaxPool1d(_MaxPoolNd):
    def forward(self, input):
        if(self._mode == 1):
            return_indices = True  ## index
            x =  F.max_pool1d(input, self.kernel_size, self.stride,
                            self.padding, self.dilation, self.ceil_mode,
                            self.return_indices)

            self._channel_size = input.shape[2] ## save one channel size
    ...
    def getIndex(self, pos):
        channel_jump = self._index.shape[2] ## 'th channel
    ...
    def e_backward(self, input, theta, pre_out):
        #1. change index of maxpool --> index of under layer
        nrs = input.clone()
        for i in range(nrs.shape[1]): 
            nrs[0,i] = self.getIndex(int(nrs[0,i].item()))

        if self._everbose:
            print('=== MaxPool ebackwrd ===')
            print('change index example 0 index = ', input[0,0], '-->', nrs[0,0])
            print('value check 0 index = ', input[1,0], '==', pre_out.flatten()[ int(nrs[0,0].item()) ] )
            #print(nrs)
...
class my_AvgPool2d(_AvgPoolNd):
    ...
    def e_backward(self, input, theta, pre_out):
        if self._everbose:
            print('=== AvgPool ebackwrd ===')
            print("!!this version doesn't consider stride")
        return input
...
class my_kMaxPool(nn.Module):
    def __init__(self, dimension=0, topk=1):
        super(my_kMaxPool, self).__init__()
    ...
    def forward(self, input):
        index = input.topk(self.topk, self.dimension)[1] 
        index = index.sort(dim = self.dimension)[0]
        x = input.gather(self.dimension, index)
        if(self._mode == 1):
            self._ovalue = x.clone()
    ...
    def e_backward(self, input, theta, pre_out):
        ...
        pre_out = pre_out.flatten()
        nrs = input.clone()
        nrs[1,:] = pre_out[ nrs[0,:].int() ]  ## should modify for kMaxpool
