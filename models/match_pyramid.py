...
class MatchPyramid(BaseModel):
    ...
    def grad_forward(self, inputs):
        ...
        left = Variable(embed_left, requires_grad=True)
        right= Variable(embed_right, requires_grad=True)
        # Compute matching signal
        # shape = [B, 1, L, R]
        embed_cross = self.matching(left, right).unsqueeze(dim=1)
        ...
        left_multi = left * left.grad
        right_multi = right * right.grad
